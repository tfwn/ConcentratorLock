...
/************************************************************************************************
* Function Name: Gprs_SetPkgProperty
* Decription   : 设置包属性值
* Input        : PkgXor-报文与运营商编码不异或标志: 0-不异或 1-异或
*				 AddrSize-地址长度 0-6字节  1-8字节
...
* Others       : 无
************************************************************************************************/
uint8 Gprs_SetPkgProperty(bool PkgXor, bool AddrSize, bool NeedAck, bool PkgType, bool Dir)
{
    PKG_PROPERTY pkgProp;

    pkgProp.Content = 0;
	pkgProp.AddrSize = AddrSize;
...
void Gprs_CmdPkg(uint8 cmd, uint8 *BufPtr)
{
    ...
    ((uint16 *)(&txPortBufPtr->Buffer[txPortBufPtr->Length]))[0] = LONG_ADDR_SIZE + DATA_FIXED_AREA_LENGTH;
    txPortBufPtr->Length += 2;
    txPortBufPtr->Buffer[txPortBufPtr->Length++] = Gprs_SetPkgProperty(XOR_OFF, EIGHT_SIZE, NONE_ACK, CMD_PKG, UP_DIR);// 报文标识(上行 + 8字节地址)
    ...
    crc16 = CalCrc16((uint8 *)(&txPortBufPtr->Buffer[tmp]), txPortBufPtr->Length - tmp);    // Crc16校验
...
void Gprs_Task(void *p_arg)
{
    ...
    while (TRUE) {
        ...
        case GprsCheckConnect:
                ...
                if (Quectel_M35 == Gprs.ModuleType) {
                    strcpy((char *)cmd, "AT+QISEND=23");
                } else {
                    strcpy((char *)cmd, "AT+QISEND=0,23");
                }
                ...
                for (loop = 0; loop < 5; loop++) {
                    ...
                    bufPtr = (PORT_BUF_FORMAT *)msg;
                    uint16 crc16 = (uint16)((bufPtr->Buffer[0x15-2]&0xff)|((bufPtr->Buffer[0x15-3] << 8)&0xff00));
                    if(( 0x15 == ((uint16 *)bufPtr->Buffer)[0] & 0x03FF ) &&
                        ( Gprs_Heartbeat_Cmd == (COMMAND_TYPE)(bufPtr->Buffer[4])) &&
                        ( 0 == memcmp(Concentrator.LongAddr, &bufPtr->Buffer[8], LONG_ADDR_SIZE) ) &&
                        ( TAILBYTE == bufPtr->Buffer[0x15-1] ) &&
                        ( crc16 == CalCrc16(bufPtr->Buffer, 0x15 - 3))){
                            Gprs.HeartbeatRetryTime = 0;
